...
- Reorder (2026-06): admins can reorder sections in edit mode via a drag handle (native HTML5 DnD) AND accessible Up/Down buttons (mobile/older-user friendly). moveSection swaps adjacent, reorder splices for drag; order persists to content and reflects on the view page. data-testids: drag-section / move-up-section / move-down-section-<scope>-<mi>-<id>. Verified iteration_23.json PASS (Up/Down + persistence; native drag not automatable but implemented).

## Rich Text Editor for description/notes fields (2026-06)
- Request: "The description and [multiline] fields should use a rich text editor so people can format and add hyperlinks." Scope (user-confirmed): ALL multi-line EArea fields across the portal (Dashboard, Reunion, Family Business, Payments, Documents, Meeting sections/agenda/purpose/decisions). Single-line EText fields stay plain. Toolbar: Headings (H2/H3), Bold, Italic, Bullet & Numbered lists, Insert/edit hyperlink (opens new tab).
- Implementation: TipTap 3.31.3 (@tiptap/react + starter-kit with link:false + extension-link) + dompurify 3.4.16. New components/RichText.jsx (RichTextEditor + sanitized RichTextView) + components/richtext.css. Editable.jsx EArea now renders RichTextEditor (edit) / RichTextView (view). Values stored as HTML strings in the loose content dict (no backend change); plain-text legacy values render fine (backward compatible). Contenteditable data-testid = edit-<path>; toolbar suffixes -h2/-h3/-bold/-italic/-bullet/-ordered/-link; view testid content-<path>. Links sanitized via DOMPurify, forced target=_blank rel=noopener.
- Verified iteration_24.json PASS (100% frontend): formatting + hyperlink dialog + persistence + hard-reload + backward compat + EText regression + no console errors.
